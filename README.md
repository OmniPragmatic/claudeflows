...
<img width="1280" height="462" alt="590113227-85c27ee8-87ba-4cbc-8e3e-05e6e75129f0" src="https://github.com/user-attachments/assets/f8f792f9-4d13-471e-a613-fb863a21e239" />

A collection of reusable workflows for software development and architecting.
...
> `brainstorm` produces a **concept**. `prd` turns a concept into **requirements**. `feature` and `bug` produce **code**. `precommit-review` produces **validated code**. Each upstream stage feeds the next, but you don't have to start at the top.
...
| A bug or a `#<issue>` | `bug` | A failing-test-driven fix, ready to commit. |
...
The skills include a routing hint in `feature`: if your repo has its own stack-specific commands (e.g. `/new-module`, `/new-migration`, `/new-worker`), the skill suggests them instead of running its generic feature flow. Project-specific commands stay in your `.claude/commands/` and don't need to be part of this plugin.

## Local development

```sh
git clone https://github.com/<your-fork>/claudeflows
cd /path/to/your-test-repo
claude --plugin-dir /path/to/claudeflows/plugins/claudeflows
```
